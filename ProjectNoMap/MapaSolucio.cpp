...
CamiBase* MapaSolucio::buscaCamiMesCurt(PuntDeInteresBase* from, PuntDeInteresBase* to)
{
    ...
    bt->nodeMesProper(from->getCoord(), nodeFrom, bt->getArrel());
    bt->nodeMesProper(to->getCoord(), nodeTo, bt->getArrel());

    construeixGraf();
    Util::escriuEnMonitor("Calculant ruta més curta...");
    CamiBase* camiMesCurt = new CamiSolucio();
    m_graf->camiMesCurt(nodeFrom, nodeTo, camiMesCurt);
    if (camiMesCurt != nullptr)
        Util::escriuEnMonitor("Calculant ruta més curta calculada correctament");
    else
        Util::escriuEnMonitor("Hi ha hagut algun error");
